...
export default function ProductDetail({ product }) {
  ...
  const handleAddToCart = () => { setCartMode("cart"); setCartOpen(true); };
  const handleBuyNow = () => { setCartMode("buynow"); setCartOpen(true); };
                ...
                {product.images.map((src, i) => (
                  <button
                    ...
                    className={`shrink-0 w-[70px] h-[70px] sm:w-[90px] sm:h-[90px] rounded-lg border-2 bg-[#111] overflow-hidden transition-all ${
                      activeImg === src ? "border-red-600" : "border-transparent hover:border-white/20"
                    }`}
                  >
                    <Image
                      src={src}
...
<p className="global-text-style text-xs sm:text-sm md:text-base">
  {product.title}
              ...
              {/* Flavours */}
              <div>
                <p className="text-red-600 font-bold text-xs uppercase tracking-[2px] mb-2">FLAVOURS</p>
                <div className="flex gap-2 flex-wrap">
                  {product.flavours.map((f) => (
                    <button
                      key={f.name}
                      onClick={() => { setSelectedFlavour(f); setActiveImg(f.images[0]); }}
              ...
              {/* Servings */}
              <div>
                <p className="text-red-600 font-bold text-xs uppercase tracking-[2px] mb-2">SERVINGS</p>
                <div className="flex gap-2 flex-wrap">
              ...
              <div className="pt-3 border-t border-[#2a2a2a]">
                <p className="text-red-600 font-bold text-xs uppercase tracking-[2px] mb-3">QUANTITY</p>
    ...
    Buy Now
              ...
              <div className="border-t border-[#2a2a2a] pt-3">
                ...
                >
                  ...
                  <span className={`transition-transform duration-300 ${factsOpen ? "rotate-45" : ""}`}>
                    <PlusIcon />
                  </span>
                </div>
                {factsOpen && (
                  <div className="global-text-style pb-4 text-sm">
